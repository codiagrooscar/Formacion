...
export default function App() {
  ...
  useEffect(() => {
    const initApp = async () => {
      try {
        setIsLoading(true);
        const [loadedTrainingsRaw, loadedEvals, loadedFollowups, loadedSettings] = await Promise.all([
        ...
        const loadedTrainings = AppStorage.reconcileTrainingsWithEvaluations(loadedTrainingsRaw, loadedEvals);
  ...
  const handleDeleteStudentData = async (studentName: string, studentEmail?: string) => {
    const normName = (studentName || '').trim().toLowerCase();
    const normEmail = (studentEmail || '').trim().toLowerCase();

    // 1. Delete matching evaluations
    const currentEvals = await AppStorage.getEvaluations();
    const evalsToDelete = currentEvals.filter(e => {
      const eName = (e.employeeName || '').trim().toLowerCase();
      const eEmail = (e.employeeEmail || '').trim().toLowerCase();
      const nameMatch = normName && (eName === normName || eName.normalize('NFD').replace(/[\u0300-\u036f]/g, '') === normName.normalize('NFD').replace(/[\u0300-\u036f]/g, ''));
      const emailMatch = normEmail && !normEmail.includes('@codiagro.com') && eEmail === normEmail;
      return nameMatch || emailMatch;
    });

    for (const ev of evalsToDelete) {
      await AppStorage.deleteEvaluation(ev.id);
    }

    // 2. Delete matching followups
    const currentFollowups = await AppStorage.getFollowups();
    const followupsToDelete = currentFollowups.filter(f => {
      const fName = (f.employeeName || '').trim().toLowerCase();
      return normName && (fName === normName || fName.normalize('NFD').replace(/[\u0300-\u036f]/g, '') === normName.normalize('NFD').replace(/[\u0300-\u036f]/g, ''));
    });

    for (const f of followupsToDelete) {
      await AppStorage.deleteFollowup(f.id);
    }

    // 3. Remove attendee from training actions
    const allTrainings = await AppStorage.getTrainingActions();
    for (const t of allTrainings) {
      if (t.attendees && t.attendees.length > 0) {
        const hasAttendee = t.attendees.some(att => {
          const attName = (att.name || '').trim().toLowerCase();
          const attEmail = (att.email || '').trim().toLowerCase();
          const nameMatch = normName && (attName === normName || attName.normalize('NFD').replace(/[\u0300-\u036f]/g, '') === normName.normalize('NFD').replace(/[\u0300-\u036f]/g, ''));
          const emailMatch = normEmail && !normEmail.includes('@codiagro.com') && attEmail === normEmail;
          return nameMatch || emailMatch;
        });

        if (hasAttendee) {
          const updatedAttendees = t.attendees.filter(att => {
            const attName = (att.name || '').trim().toLowerCase();
            const attEmail = (att.email || '').trim().toLowerCase();
            const nameMatch = normName && (attName === normName || attName.normalize('NFD').replace(/[\u0300-\u036f]/g, '') === normName.normalize('NFD').replace(/[\u0300-\u036f]/g, ''));
            const emailMatch = normEmail && !normEmail.includes('@codiagro.com') && attEmail === normEmail;
    ...
    // 4. Reload all states from storage
    const [updatedT, updatedE, updatedF] = await Promise.all([
      AppStorage.getTrainingActions(),
      AppStorage.getEvaluations(),
      AppStorage.getFollowups()
    ]);
    setTrainings(updatedT);
    setEvaluations(updatedE);
    setFollowups(updatedF);
  };

  const handleRegisterEmployee = async (employeeData: { name: string; email: string; department: string }) => {
    const newEmployee = {
      id: `emp-${Date.now()}`,
      name: employeeData.name.trim(),
      email: employeeData.email?.trim() || `${employeeData.name.toLowerCase().replace(/\s+/g, '.')}@codiagro.com`,
      department: employeeData.department || 'Producción e Ingeniería Agronómica',
      active: true
    };
