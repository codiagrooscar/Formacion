...
export const TrainingActionsList: React.FC<TrainingActionsListProps> = ({
  ...
  // Helper to reliably find all evaluations belonging to a training action
  const getEvaluationsForTraining = (t: TrainingAction) => {
    return evaluations.filter((e) => {
      if (e.trainingActionId && e.trainingActionId === t.id) return true;
      if (e.trainingCode && t.code && String(e.trainingCode).trim() === String(t.code).trim()) return true;
      if (e.trainingTitle && t.title && e.trainingTitle.trim().toLowerCase() === t.title.trim().toLowerCase()) return true;
      return false;
    });
  };
  ...
  const courseEvaluations = useMemo(() => {
    ...
    return getEvaluationsForTraining(selectedCourseForDetail);
  }, [selectedCourseForDetail, evaluations]);

  const drawerAvgSatisfaction = useMemo(() => {
    if (courseEvaluations.length > 0) {
      const sum = courseEvaluations.reduce((acc, curr) => acc + (curr.ratings?.weightedScore || curr.ratings?.overallSatisfaction || 0), 0);
      return (sum / courseEvaluations.length).toFixed(2);
    }
    if (selectedCourseForDetail?.averageSatisfaction) {
      return Number(selectedCourseForDetail.averageSatisfaction).toFixed(2);
    }
    return null;
  }, [courseEvaluations, selectedCourseForDetail]);
    ...
    <div id="trainings-list-container" className="space-y-6 pb-12">
            ...
            <tbody className="divide-y divide-[#1A2B44]/60">
              {filteredTrainings.length === 0 ? (
                ...
                filteredTrainings.map((training) => {
                  ...
                  const courseEvals = getEvaluationsForTraining(training);
                  const evalsCount = courseEvals.length > 0 ? courseEvals.length : (training.evaluationsCount || 0);
                  const avgSatisfaction = courseEvals.length > 0
                    ? (courseEvals.reduce((acc, curr) => acc + (curr.ratings?.weightedScore || curr.ratings?.overallSatisfaction || 0), 0) / courseEvals.length).toFixed(2)
                    : (training.averageSatisfaction ? Number(training.averageSatisfaction).toFixed(2) : null);
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-3 text-center">
                        ...
                        <div className={`text-[10px] font-semibold ${evalsCount > 0 ? 'text-[#00c282]' : 'text-slate-500'}`}>
                          {evalsCount} eval.
                        </div>
                      </td>
                      ...
                      <td className="py-3.5 px-3 text-center" onClick={(e) => e.stopPropagation()}>
                        <select
                          ...
                          onChange={(e) => {
                            const newStatus = e.target.value as 'planned' | 'in_progress' | 'completed';
                            onSaveTraining({
                              ...training,
                              status: newStatus
                            });
                          }}
                          className={`cursor-pointer px-2.5 py-1 rounded-full text-[11px] font-bold outline-none appearance-none text-center ${
                            training.status === 'completed' 
                              ? 'bg-emerald-500/15 text-[#00c282] border border-emerald-500/30 hover:bg-emerald-500/25' 
                              : training.status === 'in_progress'
                              ? 'bg-cyan-500/15 text-cyan-400 border border-cyan-500/30 hover:bg-cyan-500/25'
                              : 'bg-[#182840] text-slate-300 border border-[#243a5e] hover:bg-[#203454]'
                          }`}
                        >
                          <option value="planned" className="bg-[#101c2e] text-slate-300">Planificado</option>
                          <option value="in_progress" className="bg-[#101c2e] text-cyan-400">En curso</option>
                          <option value="completed" className="bg-[#101c2e] text-[#00c282]">Realizado</option>
                        </select>
                      </td>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 my-4">
              ...
              <div className="bg-amber-500/15 p-3 rounded-xl border border-amber-500/30 text-center">
                ...
                <div className="text-base font-bold text-amber-300 mt-0.5">
                  {drawerAvgSatisfaction ? `★ ${drawerAvgSatisfaction}` : 'Sin evaluar'}
                </div>
              </div>
            </div>
                    ...
                    <div key={evalItem.id} className="bg-[#0A1220] rounded-xl p-4 border border-[#1A2B44] text-xs">
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => setSelectedEvaluationForEmail(evalItem)}
                            className="p-1 rounded-md bg-blue-500/10 hover:bg-blue-500/25 text-blue-400 border border-blue-500/30 transition cursor-pointer"
                            title="Enviar Justificante Oficial por Correo Electrónico"
                          >
      ...
      {selectedEvaluationForEmail && (
        <SingleEvaluationEmailModal
          ...
          training={selectedCourseForDetail || trainings.find(t => t.id === selectedEvaluationForEmail.trainingActionId)}
          settings={settings}
        />
      )}
