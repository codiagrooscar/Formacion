...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  // Helper: check if a course is completed or has evaluations
  const isCourseExecuted = (t: TrainingAction) => {
    return (
      t.status === 'completed' ||
      (t.evaluationsCount !== undefined && t.evaluationsCount > 0) ||
      evaluations.some(e => 
        (e.trainingActionId && e.trainingActionId === t.id) ||
  ...
  const completedOrActive = filteredTrainings.filter(isCourseExecuted).length;
  ...
  const totalHoursImparted = useMemo(() => {
    return filteredTrainings
      .filter(isCourseExecuted)
      .reduce((sum, t) => sum + (t.durationHours || 0), 0);
  }, [filteredTrainings, evaluations]);
    ...
    return filteredTrainings
      ...
      .reduce((sum, t) => {
        const studentCount = (t.attendees && t.attendees.length > 0)
          ? t.attendees.length
          : (t.totalParticipantsAttended || t.totalParticipantsPlanned || 1);
  ...
  const attendanceRate = useMemo(() => {
    const executedCourses = filteredTrainings.filter(isCourseExecuted);
    ...
    const totalRealAttendance = executedCourses.reduce((sum, t) => {
      const attended = (t.attendees && t.attendees.length > 0)
        ? (t.attendees.filter(a => a.hasCompletedEvaluation).length || t.totalParticipantsAttended || t.attendees.length)
  ...
  const { averageSatisfaction, satisfactionBreakdown } = useMemo(() => {
    const relevantEvals = evaluations.filter((e) => {
      const matchCourse = filteredTrainings.find((t) => 
  ...
  }, [filteredTrainings, settings.competencyCatalog, evaluations]);
