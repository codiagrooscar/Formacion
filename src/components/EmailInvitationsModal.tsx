...
export default function EmailInvitationsModal({
  ...
  const [copiedAllStatus, setCopiedAllStatus] = useState(false);
  ...
  const handleCopyAllLinks = () => {
    const text = attendees.map(a => `${a.name} (${a.email}):\n${getEvaluationLink(a)}`).join('\n\n');
    navigator.clipboard.writeText(text);
    setCopiedAllStatus(true);
    setTimeout(() => setCopiedAllStatus(false), 3000);
  };
  ...
  const executeSendEmails = async (targets: TrainingAttendee[]) => {
    ...
    // Send emails in parallel with timeout safety
    await Promise.all(
      targets.map(async (attendee) => {
        try {
          ...
          if (activeTab === 'convocation') {
            ...
            if (res.ok && data.success) {
              ...
              if (data.error?.includes('535') || data.error?.includes('Username and Password')) {
                authError = true;
              }
              lastErrorMsg = data.error || 'Error al conectar con el servidor SMTP';
            }
          } else {
          ...
          lastErrorMsg = err?.message || 'Tiempo de espera agotado al conectar con el correo.';
        }
      })
    );
    ...
    if (success > 0) {
      ...
      if (authError || !settings.smtpPass) {
        setSendErrorMessage(
          'Para el envío directo de emails desde Gmail, Google requiere configurar una Contraseña de Aplicación de 16 caracteres en Ajustes ⚙️ (pestaña Servidor SMTP). Mientras tanto, puedes pulsar "Copiar Enlaces" para compartir la evaluación directamente por WhatsApp, Teams o correo.'
        );
      } else {
        setSendErrorMessage(
          `No se pudo completar el envío SMTP (${lastErrorMsg}). Puedes pulsar "Copiar Enlaces" o descargar el PDF/Word oficial para entregarlo a los alumnos.`
        );
      }
    }
  };
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 z-50">
      <div className="bg-[#101C2E] rounded-3xl max-w-4xl w-full p-4 sm:p-6 shadow-2xl border border-[#1A2B44] max-h-[94vh] flex flex-col">
        ...
        <div className="flex-1 min-h-[200px] flex flex-col overflow-hidden bg-[#070D17] border border-[#1A2B44] rounded-2xl p-3">
          <div className="flex flex-wrap items-center justify-between pb-2 mb-2 border-b border-[#1A2B44] shrink-0 gap-2">
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-3">
              {attendees.length > 0 && (
                <button
                  ...
                  className="text-[11px] font-semibold text-emerald-400 hover:text-emerald-300 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 px-2.5 py-1 rounded-lg flex items-center gap-1.5 transition cursor-pointer"
                  title="Copiar los enlaces de evaluación de todos los alumnos en el portapapeles"
                >
                  <Copy className="w-3 h-3 text-[#00c282]" />
                  {copiedAllStatus ? '¡Todos Copiados!' : 'Copiar Todos los Enlaces'}
                </button>
              )}
        ...
        {sentSuccessCount !== null && (
          <div className="mt-2.5 p-2.5 bg-emerald-500/15 border border-emerald-500/30 rounded-xl flex items-center gap-2.5 text-xs text-emerald-300 shrink-0 animate-fadeIn">
        ...
        {/* Error / Notice banner */}
        {sendErrorMessage && (
          <div className="mt-2.5 p-3 bg-amber-500/15 border border-amber-500/40 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 text-xs text-amber-200 shrink-0 animate-fadeIn">
            <div className="flex items-start gap-2">
              <AlertCircle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
              <p className="leading-relaxed text-[11px] sm:text-xs text-amber-200">{sendErrorMessage}</p>
            </div>
            <button
              type="button"
              onClick={handleCopyAllLinks}
              className="px-3 py-1.5 bg-amber-500/20 hover:bg-amber-500/30 text-amber-200 font-bold text-xs rounded-lg border border-amber-500/40 transition shrink-0 flex items-center gap-1.5 cursor-pointer"
            >
              <Copy className="w-3.5 h-3.5 text-amber-300" />
              {copiedAllStatus ? '¡Copiados!' : 'Copiar Enlaces Ahora'}
            </button>
          </div>
        )}
