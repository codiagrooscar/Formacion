...
import { SingleEvaluationEmailModal } from './SingleEvaluationEmailModal';
import { generateEvaluationPdf } from '../utils/documentGenerator';
...
interface AnalyticsViewProps {
  ...
  onDeleteStudentData?: (studentName: string, studentEmail?: string) => Promise<void> | void;
  onRegisterEmployee?: (employee: { name: string; email: string; department: string }) => Promise<void> | void;
...
interface StudentAnalytics {
  ...
  email: string;
  department: string;
  isRegisteredInSettings: boolean;
...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({
  ...
  const [selectedEvaluationForEmail, setSelectedEvaluationForEmail] = useState<Evaluation | null>(null);

  // Student Delete & Register states
  const [studentToDelete, setStudentToDelete] = useState<StudentAnalytics | null>(null);
  ...
  const filteredBaseEvaluations = useMemo(() => {
    return evaluations.filter((e) => {
      const matchCourse = trainings.find((t) => 
        (e.trainingActionId && t.id === e.trainingActionId) ||
        (e.trainingCode && t.code && String(e.trainingCode).trim() === String(t.code).trim()) ||
        (e.trainingTitle && t.title && e.trainingTitle.trim().toLowerCase() === t.title.trim().toLowerCase())
      );
  ...
  const filteredBaseFollowups = useMemo(() => {
    return followups.filter((f) => {
      const matchCourse = trainings.find((t) => 
        (f.trainingActionId && t.id === f.trainingActionId) ||
        (f.trainingCode && t.code && String(f.trainingCode).trim() === String(t.code).trim()) ||
        (f.trainingTitle && t.title && f.trainingTitle.trim().toLowerCase() === t.title.trim().toLowerCase())
      );
      if (departmentFilter !== 'all') {
  ...
  const studentsAnalytics = useMemo<StudentAnalytics[]>(() => {
    ...
    // Build set of registered employee names from settings.employees
    ...
    (settings.employees || []).forEach((emp) => {
      if (emp.active !== false) {
        if (emp.name) {
          ...
          registeredNamesSet.add(n);
          registeredNamesSet.add(n.normalize('NFD').replace(/[\u0300-\u036f]/g, ''));
        }
        if (emp.email) {
          registeredEmailsSet.add(emp.email.trim().toLowerCase());
        }
      }
    });
    ...
    studentMap.forEach((val, key) => {
      ...
      const cleanStudentName = val.name.trim().toLowerCase();
      const cleanNoAccents = cleanStudentName.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
      const isRegisteredInSettings = registeredNamesSet.size === 0 
        ? true 
        : (registeredNamesSet.has(cleanStudentName) || registeredNamesSet.has(cleanNoAccents));
  ...
  // Counts of registered vs unregistered students
  const unregisteredCount = useMemo(() => {
    return studentsAnalytics.filter((s) => !s.isRegisteredInSettings).length;
  }, [studentsAnalytics]);
  ...
  const filteredStudents = useMemo(() => {
    return studentsAnalytics
      .filter((s) => {
        ...
        if (registrationFilter === 'registered' && !s.isRegisteredInSettings) return false;
        if (registrationFilter === 'unregistered' && s.isRegisteredInSettings) return false;
        if (searchTerm) {
  ...
  }, [studentsAnalytics, departmentFilter, registrationFilter, searchTerm, sortBy]);

  // Actions for unregistered students
  const handleQuickRegister = async (student: StudentAnalytics) => {
    if (!onRegisterEmployee) return;
    ...
    try {
      ...
      showToast(`¡Alumno "${student.name}" dado de alta exitosamente en la Configuración de Empleados!`, 'success');
    } catch (err) {
      showToast('Error al dar de alta al empleado en la configuración.', 'error');
    } finally {
  ...
  const handleConfirmDeleteStudent = async () => {
    if (!studentToDelete || !onDeleteStudentData) return;
    setIsDeleting(true);
    try {
      const sName = studentToDelete.name;
      await onDeleteStudentData(studentToDelete.name, studentToDelete.email);
      if (selectedStudent && selectedStudent.id === studentToDelete.id) {
        setSelectedStudent(null);
      }
      setStudentToDelete(null);
      showToast(`Registros y datos de "${sName}" eliminados correctamente de las estadísticas.`, 'success');
    } catch (err) {
      showToast('Error al eliminar los datos del alumno de las estadísticas.', 'error');
    } finally {
      setIsDeleting(false);
    }
  };

  const handleDeleteSingleEvaluation = async (evaluationId: string) => {
    if (!onDeleteEvaluation) return;
    try {
      await onDeleteEvaluation(evaluationId);
      showToast('Cuestionario de evaluación eliminado correctamente.', 'success');
    } catch (err) {
      showToast('Error al eliminar el cuestionario de evaluación.', 'error');
    }
  };
    ...
    <div id="analytics-view-container" className="space-y-6 pb-12">
      ...
      {activeTab === 'students' && (
        <div className="bg-[#101C2E] rounded-2xl border border-[#1A2B44] shadow-sm overflow-hidden">
          <div className="p-4 sm:p-5 border-b border-[#1A2B44] flex flex-wrap items-center justify-between gap-3">
            <div>
            ...
            {/* Registration State Quick Filters */}
            <div className="flex flex-wrap items-center gap-2">
              <button
                type="button"
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer ${
                  registrationFilter === 'all'
                    ...
                    : 'bg-[#0A1220] text-slate-400 hover:text-slate-200 border border-[#1A2B44]'
                }`}
              >
                <span>Todos ({studentsAnalytics.length})</span>
              </button>
              ...
              >
                <CheckCircle2 className="w-3.5 h-3.5 text-[#00c282]" />
                <span>Dados de alta ({registeredCount})</span>
              </button>
              ...
              >
                ...
                <span>No dados de alta ({unregisteredCount})</span>
              </button>
            </div>
          </div>

          {/* Unregistered Alert Banner if any exist and filter is not active */}
          {unregisteredCount > 0 && registrationFilter === 'all' && (
            <div className="bg-amber-500/10 border-b border-amber-500/20 px-4 py-2.5 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2 text-amber-300">
                ...
                <span>
                  Hay <strong>{unregisteredCount} alumno(s)</strong> en las estadísticas que no están dados de alta en la Configuración de Empleados.
                </span>
              </div>
              <button
                onClick={() => setRegistrationFilter('unregistered')}
                className="text-amber-300 hover:text-amber-100 font-bold underline cursor-pointer text-xs"
              >
                Filtrar no dados de alta
              </button>
            </div>
          )}
              ...
              <tbody className="divide-y divide-[#1A2B44]/60">
                {filteredStudents.length === 0 ? (
                  ...
                  filteredStudents.map((s) => (
                    <tr 
                      key={s.id} 
                      className={`hover:bg-[#14233a]/60 transition ${
                        !s.isRegisteredInSettings ? 'bg-amber-500/[0.04]' : ''
                      }`}
                    >
                        ...
                        <div className="font-bold text-white flex items-center gap-2.5">
                          ...
                          <div>
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              {!s.isRegisteredInSettings && (
                                <span 
                                  className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-500/15 text-amber-300 border border-amber-500/30" 
                                  title="Este alumno no está dado de alta en la configuración oficial de empleados"
                                >
                      ...
                      <td className="py-3.5 px-4 text-center whitespace-nowrap">
                        <div className="flex items-center justify-center gap-1.5">
                          <button
                            ...
                            className="px-2.5 py-1.5 rounded-lg bg-emerald-500/15 hover:bg-emerald-500/30 text-[#00c282] border border-emerald-500/30 transition text-xs font-bold flex items-center gap-1 cursor-pointer"
                            title="Ver Expediente Formativo Completo"
                          >
                            <Eye className="w-3.5 h-3.5" />
                            <span className="hidden sm:inline">Expediente</span>
                          </button>

                          {!s.isRegisteredInSettings && onRegisterEmployee && (
                            <button
                              type="button"
                              disabled={isRegistering}
                              onClick={() => handleQuickRegister(s)}
                              className="p-1.5 rounded-lg bg-blue-500/15 hover:bg-blue-500/30 text-blue-300 border border-blue-500/30 transition cursor-pointer"
                              title="Dar de alta como empleado oficial en Configuración"
                            >
                          ...
                          {onDeleteStudentData && (
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-rose-500/15 hover:bg-rose-500/30 text-rose-300 border border-rose-500/30 transition cursor-pointer"
                              title="Borrar alumno y sus registros de las estadísticas"
                            >
      ...
      {selectedStudent && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xs flex items-center justify-center p-4 z-50 overflow-y-auto animate-fade-in">
          <div className="bg-[#101C2E] rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-[#1A2B44] max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between border-b border-[#1A2B44] pb-4">
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-2xl font-black text-lg flex items-center justify-center ${
                  !selectedStudent.isRegisteredInSettings
                    ? 'bg-amber-500/20 border border-amber-500/40 text-amber-300'
                    : 'bg-blue-500/15 border border-blue-500/30 text-blue-300'
                }`}>
            ...
            {/* Unregistered warning callout */}
            {!selectedStudent.isRegisteredInSettings && (
              <div className="mt-4 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-200">
                <div className="flex items-start gap-3">
                  <AlertTriangle className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" />
                  <div className="flex-1 text-xs">
                    <h4 className="font-bold text-amber-300 text-sm">Alumno No Dado de Alta en Configuración</h4>
                    <p className="text-slate-300 mt-1 leading-relaxed">
                      Este participante aparece en las estadísticas por cuestionarios cumplimentados o asistencias registradas, pero <strong>no figura en el listado oficial de empleados</strong> de CODIAGRO.
                    </p>
                    <div className="flex flex-wrap items-center gap-2.5 mt-3">
                      {onRegisterEmployee && (
                        <button
                          type="button"
                          disabled={isRegistering}
                          onClick={() => handleQuickRegister(selectedStudent)}
                          className="px-3 py-1.5 rounded-xl bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-200 border border-emerald-500/40 text-xs font-bold flex items-center gap-1.5 cursor-pointer"
                        >
                          <UserPlus className="w-3.5 h-3.5" />
                          <span>Dar de alta en Configuración</span>
                        </button>
                      )}
                      {onDeleteStudentData && (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-xl bg-rose-500/20 hover:bg-rose-500/30 text-rose-200 border border-rose-500/40 text-xs font-bold flex items-center gap-1.5 cursor-pointer"
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                          <span>Borrar de las estadísticas</span>
                        </button>
                      )}
                    </div>
                  </div>
            ...
            {selectedStudent.evaluations.length > 0 && (
              <div>
                <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
                  <Star className="w-3.5 h-3.5 text-amber-400" />
                  Cuestionarios de Evaluación Cumplimentados ({selectedStudent.evaluations.length})
                </h4>

                <div className="space-y-2.5 max-h-48 overflow-y-auto pr-1">
                  {selectedStudent.evaluations.map((ev) => (
                    <div key={ev.id} className="bg-[#0A1220] p-3 rounded-xl border border-[#1A2B44] text-xs flex items-center justify-between">
                      <div>
                        <div className="font-bold text-white flex items-center gap-2">
                          <span>{ev.trainingTitle || 'Acción Formativa'}</span>
                          <span className="font-mono font-semibold text-emerald-400 text-[11px]">[{ev.trainingCode || 'RE0180104'}]</span>
                        </div>
                        <div className="text-slate-400 text-[11px] mt-0.5 flex items-center gap-2">
                          <span>Fecha: {ev.submissionDate}</span>
                          <span>•</span>
                          <span className="font-bold text-amber-300">★ {ev.ratings?.overallSatisfaction || 0} / 5</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          type="button"
                          onClick={() => setSelectedEvaluationForEmail(ev)}
                          className="p-1.5 rounded-lg bg-blue-500/15 hover:bg-blue-500/30 text-blue-300 border border-blue-500/30 transition cursor-pointer"
                          title="Enviar Justificante Oficial por Email"
                        >
                          <Mail className="w-3.5 h-3.5" />
                        </button>
                        <button
                          type="button"
                          onClick={() => generateEvaluationPdf(ev, settings)}
                          className="p-1.5 rounded-lg bg-emerald-500/15 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/30 transition cursor-pointer"
                        ...
                        {onDeleteEvaluation && (
                          <button
                            type="button"
                            onClick={() => handleDeleteSingleEvaluation(ev.id)}
                            className="p-1.5 rounded-lg bg-rose-500/15 hover:bg-rose-500/30 text-rose-300 border border-rose-500/30 transition cursor-pointer"
                            title="Eliminar este cuestionario de evaluación"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        )}
            ...
            <div className="mt-6 pt-4 border-t border-[#1A2B44] flex items-center justify-between flex-wrap gap-2">
              {onDeleteStudentData ? (
                <button
                  type="button"
                  onClick={() => setStudentToDelete(selectedStudent)}
                  className="px-3.5 py-2 bg-rose-500/15 hover:bg-rose-500/25 text-rose-300 border border-rose-500/30 font-bold rounded-xl text-xs transition flex items-center gap-1.5 cursor-pointer"
                >
                  <Trash2 className="w-3.5 h-3.5" />
                  <span>Borrar de las estadísticas</span>
                </button>
      ...
      {/* Single Evaluation Email Modal */}
      {selectedEvaluationForEmail && (
        <SingleEvaluationEmailModal
          isOpen={!!selectedEvaluationForEmail}
          onClose={() => setSelectedEvaluationForEmail(null)}
          evaluation={selectedEvaluationForEmail}
          training={trainings.find(t => t.id === selectedEvaluationForEmail.trainingActionId)}
          settings={settings}
          onSaveSettings={onSaveSettings}
        />
      )}

      {/* Confirmation Modal: Delete Student Data from Statistics */}
      {studentToDelete && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-[#101C2E] border border-rose-500/30 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="p-5 sm:p-6 border-b border-[#1A2B44] flex items-center justify-between bg-rose-950/20">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-2xl bg-rose-500/20 border border-rose-500/30 text-rose-400 flex items-center justify-center shrink-0">
                  <Trash2 className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-bold text-white text-base">¿Borrar alumno de las estadísticas?</h3>
                  <p className="text-xs text-rose-300/80">Gestión y depuración de registros formativos ISO 9001</p>
                </div>
              </div>
              <button
                onClick={() => setStudentToDelete(null)}
                disabled={isDeleting}
                className="text-slate-400 hover:text-white p-1 rounded-xl hover:bg-[#182840] transition cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-5 sm:p-6 space-y-4 text-xs">
              <div className="bg-[#0A1220] p-4 rounded-2xl border border-[#1A2B44] space-y-2">
                <div className="flex justify-between items-center text-slate-300">
                  <span className="text-slate-400">Alumno:</span>
                  <strong className="text-white text-sm">{studentToDelete.name}</strong>
                </div>
                <div className="flex justify-between items-center text-slate-300">
                  <span className="text-slate-400">Email:</span>
                  <span className="font-mono text-slate-300">{studentToDelete.email}</span>
                </div>
                <div className="flex justify-between items-center text-slate-300">
                ...
                <div className="flex justify-between items-center text-slate-300">
                  <span className="text-slate-400">Estado en Configuración:</span>
                  {studentToDelete.isRegisteredInSettings ? (
                    <span className="text-emerald-400 font-bold flex items-center gap-1">
                      <CheckCircle2 className="w-3.5 h-3.5" /> Dado de alta
                    </span>
                  ) : (
                    <span className="text-amber-400 font-bold flex items-center gap-1">
                      <AlertTriangle className="w-3.5 h-3.5" /> No dado de alta
                    </span>
                  )}
                </div>
              </div>

              <div className="bg-rose-500/10 border border-rose-500/25 rounded-2xl p-4 space-y-2.5 text-rose-200">
                <h5 className="font-bold text-rose-300 flex items-center gap-2">
                  <AlertCircle className="w-4 h-4 text-rose-400" />
                  Registros que serán eliminados de la base de datos:
                </h5>
                <ul className="list-disc list-inside space-y-1 text-slate-300 text-[11px] pl-1">
                  <li>
                    <strong>{studentToDelete.evaluationsCount}</strong> cuestionario(s) de satisfacción cumplimentados.
                  </li>
                  <li>
                    <strong>{studentToDelete.coursesEnrolled.length}</strong> registro(s) de asistencia en cursos formativos.
                  </li>
                  <li>
                    <strong>{studentToDelete.followupsCount}</strong> seguimiento(s) de eficacia en el puesto.
                  </li>
                </ul>
                <p className="text-[11px] text-slate-400 pt-1 leading-relaxed border-t border-rose-500/20">
                  Esta acción eliminará estos registros de las tablas y cálculos para que horas a cero o datos de prueba no distorsionen las estadísticas oficiales.
                </p>
              </div>
            </div>

            <div className="p-4 sm:p-5 border-t border-[#1A2B44] bg-[#0A1220] flex items-center justify-end gap-2.5">
              <button
                type="button"
                disabled={isDeleting}
                onClick={() => setStudentToDelete(null)}
                className="px-4 py-2 rounded-xl bg-[#182840] hover:bg-[#203656] text-slate-300 border border-[#243a5e] font-bold text-xs cursor-pointer transition"
              >
                Cancelar
              </button>
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs flex items-center gap-2 shadow-lg shadow-rose-900/30 cursor-pointer disabled:opacity-50 transition"
              >
                {isDeleting ? (
                  <>
                  ...
                  <>
                    <Trash2 className="w-3.5 h-3.5" />
                    <span>Sí, borrar de las estadísticas</span>
                  </>
                )}
              </button>
            </div>
        ...
        <div className="fixed bottom-6 right-6 z-50 animate-in fade-in slide-in-from-bottom-5 duration-200">
          <div className={`px-4 py-3 rounded-2xl shadow-2xl border flex items-center gap-3 text-xs font-bold ${
            ...
            {toastMessage.type === 'error' ? (
              ...
              <CheckCircle2 className="w-4 h-4 text-[#00c282] shrink-0" />
            )}
