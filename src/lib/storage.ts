...
export class AppStorage {
  ...
  static async getTrainingActions(): Promise<TrainingAction[]> {
    ...
    let evalsList: Evaluation[] = [];
    try {
      const rawEvals = localStorage.getItem(LOCAL_STORAGE_KEYS.EVALUATIONS);
      if (rawEvals) evalsList = JSON.parse(rawEvals);
    } catch {}

    list = this.reconcileTrainingsWithEvaluations(list, evalsList);
  ...
  // Pure reconciliation logic for trainings & evaluations
  static reconcileTrainingsWithEvaluations(trainings: TrainingAction[], evals: Evaluation[]): TrainingAction[] {
    return trainings.map((t, idx) => {
      ...
      const matchedEvals = evals.filter((e) => 
        (e.trainingActionId && e.trainingActionId === updatedCourse.id) ||
        (e.trainingCode && updatedCourse.code && String(e.trainingCode).trim() === String(updatedCourse.code).trim()) ||
        (e.trainingTitle && updatedCourse.title && e.trainingTitle.trim().toLowerCase() === updatedCourse.title.trim().toLowerCase())
      );

      if (matchedEvals.length > 0) {
        const sumScore = matchedEvals.reduce((acc, curr) => acc + (curr.ratings?.weightedScore || curr.ratings?.overallSatisfaction || 0), 0);
        updatedCourse.evaluationsCount = matchedEvals.length;
        updatedCourse.averageSatisfaction = Number((sumScore / matchedEvals.length).toFixed(2));
        
        const totalApp = matchedEvals.reduce((acc, curr) => acc + (curr.ratings?.attendeeEffectiveness?.practicalUtility || curr.ratings?.jobApplicability || 4), 0);
        const avgApp = totalApp / matchedEvals.length;
        const effScore = Math.min(100, Math.round((avgApp / 5) * 100));
        updatedCourse.effectivenessScore = effScore;
        updatedCourse.isEffective = effScore >= 75;

        // Auto promote status if still marked as planned
        if (updatedCourse.status === 'planned') {
          updatedCourse.status = 'completed';
        }

        if (updatedCourse.attendees && updatedCourse.attendees.length > 0) {
          updatedCourse.attendees = updatedCourse.attendees.map(att => {
            const hasDone = matchedEvals.some(me => 
              (me.employeeEmail && att.email && me.employeeEmail.toLowerCase() === att.email.toLowerCase()) ||
              (me.employeeName && att.name && me.employeeName.toLowerCase().trim() === att.name.toLowerCase().trim())
            );
            return hasDone ? { ...att, hasCompletedEvaluation: true } : att;
          });
        }

        const attendedCount = Math.max(
          updatedCourse.totalParticipantsAttended || 0,
          (updatedCourse.attendees?.filter(a => a.hasCompletedEvaluation)?.length || 0),
          matchedEvals.length
        );
        updatedCourse.totalParticipantsAttended = attendedCount;
      }

      if (updatedCourse.attendees && updatedCourse.attendees.length > 0) {
        updatedCourse.totalParticipantsPlanned = Math.max(updatedCourse.totalParticipantsPlanned || 0, updatedCourse.attendees.length);
      }

      const planned = updatedCourse.totalParticipantsPlanned || (updatedCourse.attendees?.length) || 1;
      const attended = updatedCourse.totalParticipantsAttended || 0;
      updatedCourse.attendanceRate = Math.min(100, Math.round((attended / planned) * 100));
  ...
  static async addEvaluation(evalData: Omit<Evaluation, 'id'>): Promise<Evaluation> {
    ...
    const courseTrain = allTrainings.find((t) => 
      (t.id && t.id === evalData.trainingActionId) ||
      (t.code && evalData.trainingCode && String(t.code).trim() === String(evalData.trainingCode).trim()) ||
      (t.title && evalData.trainingTitle && t.title.trim().toLowerCase() === evalData.trainingTitle.trim().toLowerCase())
    );
    
    const newEval: Evaluation = { 
      ...evalData, 
      id,
      trainingActionId: courseTrain?.id || evalData.trainingActionId,
      trainingCode: evalData.trainingCode || courseTrain?.code || '26001',
      trainingTitle: evalData.trainingTitle || courseTrain?.title || ''
    };
    ...
    if (courseTrain) {
      const courseEvals = updatedEvals.filter((e) => 
        (e.trainingActionId && e.trainingActionId === courseTrain.id) ||
        (e.trainingCode && courseTrain.code && String(e.trainingCode).trim() === String(courseTrain.code).trim()) ||
        (e.trainingTitle && courseTrain.title && e.trainingTitle.trim().toLowerCase() === courseTrain.title.trim().toLowerCase())
      );
      ...
      let updatedAttendees = courseTrain.attendees;
      if (updatedAttendees && updatedAttendees.length > 0) {
        updatedAttendees = updatedAttendees.map(att => {
          const hasDone = courseEvals.some(me => 
            (me.employeeEmail && att.email && me.employeeEmail.toLowerCase() === att.email.toLowerCase()) ||
            (me.employeeName && att.name && me.employeeName.toLowerCase().trim() === att.name.toLowerCase().trim())
          );
          return hasDone ? { ...att, hasCompletedEvaluation: true } : att;
